...
contract VickreyUtilitiesBase {
    ...
    returns (EthereumDecoder.BlockHeader memory header)
  {
    ...
    while (it.hasNext()) {
      ...
      else if (idx == 16) header.withdrawalsRoot = bytes32(it.next().toUint());
      else if (idx == 17) header.blobGasUsed = (it.next().toUint());
      else if (idx == 18) header.excessBlobGas = (it.next().toUint());
      else if (idx == 19) header.parentBeaconBlockRoot = bytes32(it.next().toUint());
      else it.next();
      idx++;
    }
